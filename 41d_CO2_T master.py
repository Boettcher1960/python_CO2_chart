...
# 0.3.1 scale the plot field
y_min = 300 # min value 280
y_max = 750 # min value 440 70

y_Tmin = 0 # min value °C
y_Tmax = 9 # 40 # max value C

x_anf = 2000 # 1960 geht, 2000 geht
...
ax1.set_ylabel("CO₂ in ppm", color="blue", fontsize=16) # y achse links

# 1.7.4 write the numbers left of plot field
ax1.tick_params(axis="y", labelcolor="blue", labelsize=16) # Achsenbeschriftung
ax1.grid(True)
...
if plot6_Glen_CO2_on > 0:
   ax6.plot(df6["Year6"], df6["Modeled6"], '--', label="Glen formula CO2= 0.0132t² - 51t + 49,536 K6", color="red", linewidth=3)
...
if plot7_temperature > 0:
   ...
   ax7.plot(df7["Year7"], df7["Modeled7"], '--', label="T formula CO2=  K6", color="purple", linewidth=3)
   ...
   # ax7.grid(True, which="major") # Netz # 120 / 20 = 6 y_block
   # ax7.set_ylim(y_Tmin, y_block * 0.5 ) # scale
   ax7.set_ylim(y_Tmin, 3 ) # scale
   # ax7.grid(True, which="minor", alpha=0.3)
...
if plot7_temperature > 0:
   ...
   ax7.set_ylabel("Δ Temperature in °C", color="purple", fontname="Arial",fontsize=18) # fontweight="bold"
   ax7.tick_params(axis="y", labelcolor="purple", labelsize=16)
...
if plot7_temperature > 0: # one temperature active
   ...
   plt.text(-0.1, 1.05,
     f"{x_anf} CO2 in atmosphere",    # f"More CO2 in atmosphere", 
     color="blue", fontname="Arial", fontsize=trs, transform=plt.gca().transAxes )
   plt.text(0.24, 1.05,
     f"- - CO2 quadratic",
     color="red", fontname="Arial", fontsize=trs, transform=plt.gca().transAxes )
   plt.text(0.48, 1.05,
     f"--> Δ Temperature calculated °C year 2100",
     color="purple", fontname="Arial", fontsize=trs, transform=plt.gca().transAxes )
else:   
...
if plot6_Glen_CO2_on == 3: # print in line 3
   ...
   # 9.2.4 write blue text
   red_text="Red dashed @gergyl.bsky atmosphere ppm = 0.0132251t² - 51.0337t + 49,536"
   # 9.2.5 plot the blue text
   plt.text(tr2x, tr3y, red_text, color="red", fontname="Arial", fontsize=trs,
   transform=plt.gca().transAxes)
...
# 9.4.6_ print line 3 red Glen data below the figure
   ...
   fig.add_artist(line4)
   # 9.2.4 write blue text
   red_text="Red dashed line: Glen parabol formula ppm = 0.0132251t² - 51.0337t + 49,536"
   # 9.2.5 plot the blue text
   plt.text(tr2x, tr4y, red_text, color="red", fontname="Arial", fontsize=trs,
   transform=plt.gca().transAxes)
elif plot7_temperature == 4:
   line7 = Line2D([lr2x1, lr2x2], [lr4y, lr4y], # y from 0 to 1
   transform=fig.transFigure,
   marker="o", markersize=3, color="purple", linewidth=2)
   ...
   red_text="purple @reescatophuls.bsky :  Temperature = 0.000618t² - 2.459 t + 2446.0579"
   # 9.2.5 plot the blue text
   plt.text(tr2x, tr4y, red_text, color="purple", fontname="Arial", fontsize=trs,
   transform=plt.gca().transAxes)
...
header_black = "      CO2 values are in ppm.  Plot-Parameter="
...
#text6 = text6 + f" ydiff= {ydiff}" # y_max number inside string yr0
#text6 = text6 + f" yr0= {yr0}" # y_max number inside string yr0
#text6 = text6 + f" yr1= {yr1} " # y_max number inside string yr0
text6 = text6 + header_black

# 9.6 plot line 6
plt.text(-0.05, -.56, text6 , color="black", fontname="Arial", fontsize=trs,
    transform=plt.gca().transAxes)
# plt.title(string, color="blue", fontname="Arial",fontsize=18)
fig.tight_layout()
plt.tight_layout()
plt.show()

# 9.7 save the plot line 6
#filename = "39g3 figure "
